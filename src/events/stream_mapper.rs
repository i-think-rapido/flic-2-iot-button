  ...
  fn read_event(&mut self) -> Event {
    ...
    match self.read_u8() {
      Some(opcode) => match opcode.try_into().ok() {
        ...
        Some(OpCode::ScanWizardCompleted) => match (
          self.read_u32(),
          self.read_enum(),
        ) {
          (
            ...
            match result.try_into().ok() {
              Some(result) =>
                Event::ScanWizardCompleted {
                  scan_wizard_id,
                  result,
                },
              _ => Event::CorruptEvent,
        ...
        Some(OpCode::ButtonDeleted) => match (
          self.read_bdaddr(),
          self.read_bool(),
        ) {
          (
            Some(bd_addr),
            Some(deleted_by_this_client),
          ) => Event::ButtonDeleted {
            bd_addr,
        ...
        Some(OpCode::BatteryStatus) => match (
          self.read_u32(),
          self.read_i8(),
          self.read_u64(),
        ) {
